...
export const getNetworkGraphData = (dispatch) => {
  return ({ host, network, networkLastTime }) => {
    const networkPath = `http://18.236.126.230/render?target=icinga2.${host}.services.${network}&from=${networkLastTime}&format=json`;

    console.log("fetching network graph data...");
...
export const getMemoryGraphData = (dispatch) => {
  return ({ host }) => {
    ...
    console.log("fetching memory graph data...");
